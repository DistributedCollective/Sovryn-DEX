...
import { TimelockAccepts, SdexPolicy, SdexSwapDex } from "../../typechain"
import { BOOT_PROXY_IDX, COLD_PROXY_IDX, SdexAddrs, SdexGovAddrs, FLAG_CROSS_PROXY_IDX, KNOCKOUT_LP_PROXY_IDX, LONG_PROXY_IDX, LP_PROXY_IDX, MICRO_PROXY_IDX, SAFE_MODE_PROXY_PATH, SWAP_PROXY_IDX } from "../constants/addrs"
...
export interface SdexProtocolCmd {
    callpath: number,
...
export async function opsResolution (addrs: SdexAddrs, cmd: SdexProtocolCmd, 
    ...
    const policy = await refContract("SdexPolicy", addrs.policy) as SdexPolicy
...
export async function treasuryResolution (addrs: SdexAddrs, cmd: SdexProtocolCmd, 
    delay: number, tag: string): Promise<GovernanceResolution> {
...
export async function opsTimelockSet (addrs: SdexAddrs, newDelay: number, oldDelay: number) {
...
export async function treasuryTimelockSet (addrs: SdexAddrs, newDelay: number, oldDelay: number) {
...
export function printResolution (res: GovernanceResolution, tag: string): GovernanceResolution {
    ...
    console.log()
    console.log(`Will execute a protocolCmd() call on SdexSwapDex contract at ${res.dexContract}`)
...
export async function decodePolicySched (timelock: TimelockAccepts, policy: SdexPolicy,
    dex: SdexSwapDex, schedData: string) {
    ...
    if (schedCall.target.toLowerCase() !== policy.address.toLowerCase()) {
        throw new Error("Target of schedule call is not SdexPolicy contract")
    }
    ...
    if (policyCall.minion.toLowerCase() !== dex.address.toLowerCase()) {
        throw new Error("Target of SdexPolicy call is not SdexSwapDex contract")
    }
    ...
    if (callCode === PROTOCOL_AUTH_TRANSFER_CMD && policyCall.proxyPath === COLD_PROXY_IDX) {
        console.log(`Decoded protocolCmd as AuthTransfer to new SdexPolicy`)
