...
import { SdexSwapDex } from '../../contracts/typechain/SdexSwapDex';
...
async function createPool (token: MockBypassERC20, base: MockBypassERC20, price: number, dex: SdexSwapDex): Promise<void> {
...
async function mintLiq (token: MockBypassERC20, base: MockBypassERC20, dex: SdexSwapDex, query: QueryHelper): Promise<void> {
    let baseDecs = await base.decimals()
    let quoteDecs = await token.decimals()
...
async function displayPrice (token: MockBypassERC20, base: MockBypassERC20, query: QueryHelper): Promise<void> {
    ...
    let sdexPrice = (addrLessThan(base.address, token.address)) ?
    ...
    let divPrice = (sdexPrice / Math.pow(2, 64)) * (sdexPrice / Math.pow(2, 64))
    if (!addrLessThan(base.address, token.address)) {
...
async function inflate() {
    ...
    //let dex = factory.attach("0xDc64a140Aa3E981100a9becA4E685f962f0cF6C9") as SdexSwapDex
    let dex = factory.attach("0x6c40E8A335bF5956DeBD2FB88D5c98Cc0A760559") as SdexSwapDex
