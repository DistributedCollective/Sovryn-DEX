...
async function validate() {
    ...
    validateDeploy(addrs.dex, "SdexSwapDex", provider),
    validateDeploy(addrs.policy, "SdexPolicy", provider, addrs.dex)]
    await Promise.all(pending)
    console.log()

    console.log("Validating Peripheral Contracts...")
    pending = [
    validateDeploy(addrs.impact, "SdexImpact", provider, addrs.dex),
    validateDeploy(addrs.query, "SdexQuery", provider, addrs.dex)]
    await Promise.all(pending)
    console.log()
}
