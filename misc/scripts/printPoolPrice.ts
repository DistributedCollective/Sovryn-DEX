...
function isValidPrice(price: number): boolean {
    return price !== 0 && price !== Infinity && !isNaN(price);
}
    ...
    for (const pool of poolConfigs) {
        try {
            ...
            if (!token1.address || !token2.address) {
                console.log(
                    `Skipping ${pool.baseToken.tokenSymbol}/${pool.quoteToken.tokenSymbol} - Missing token address`
                );
                continue;
            }
            ...
            // Get decimals
            const token1Decimals = pool.baseToken.isNativeToken
                ? 18
                : await token1Contract.decimals();
            const token2Decimals = pool.quoteToken.isNativeToken
                ? 18
            ...
            const [baseToken, quoteToken, baseSymbol, quoteSymbol, baseDecimals, quoteDecimals] =
                comparison > 0
                    ? [
                          token2.address,
                          token1.address,
                          pool.quoteToken.tokenSymbol,
                          pool.baseToken.tokenSymbol,
                          token2Decimals,
                          token1Decimals,
                      ]
                    : [
            ...
            if (!isValidPrice(adjustedPrice)) {
                console.warn("Status: Pool does not exist or has no liquidity");
                console.warn(`Pool Index: ${pool.poolIdx}`);
                console.warn(
                    `Base Token (${baseSymbol}): ${baseToken} (${baseDecimals} decimals)`
                );
                console.warn(
                    `Quote Token (${quoteSymbol}): ${quoteToken} (${quoteDecimals} decimals)`
                );
                continue;
            }
            ...
            console.error(
                `Error querying ${pool.baseToken.tokenSymbol}/${pool.quoteToken.tokenSymbol}:`,
