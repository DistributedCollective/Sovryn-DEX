...
import { BOOT_PROXY_IDX, COLD_PROXY_IDX, KNOCKOUT_LP_PROXY_IDX, LONG_PROXY_IDX, LP_PROXY_IDX, SAFE_MODE_PROXY_PATH, SWAP_PROXY_IDX, buildSdexSwapDex } from "./SetupDex";
import { SdexSwapRouter, SdexSwapRouterBypass } from "../typechain";
...
export async function makePoolFrom (tokenX: Token, tokenY: Token, dex?: SdexSwapDex): Promise<TestPool> {
...
export class TestPool {
    dex: Promise<SdexSwapDex>
    query: Promise<SdexQuery>
    router: Promise<SdexSwapRouter>
    routerBypass: Promise<SdexSwapRouterBypass>
    ...
    constructor (base: Token, quote: Token, dex?: SdexSwapDex) {
        this.base = base
        this.quote = quote
        ...
        factory = ethers.getContractFactory("SdexSwapDexSeed")
        if (dex) {
            this.dex = Promise.resolve(dex).then(d => {
                this.setupLpTokenBeaconAddress(d)
                return d;
            })
        } else {
            this.dex = buildSdexSwapDex(this.auth)
        }

        factory = ethers.getContractFactory("SdexQuery")
        this.query = factory.then(f => this.dex.then(
            d => f.deploy(d.address))) as Promise<SdexQuery>

        factory = ethers.getContractFactory("SdexSwapRouter")
        this.router = factory.then(f => this.dex.then(
            d => f.deploy(d.address))) as Promise<SdexSwapRouter>

        factory = ethers.getContractFactory("SdexSwapRouterBypass")
        this.routerBypass = factory.then(f => this.dex.then(
            d => f.deploy(d.address))) as Promise<SdexSwapRouterBypass>
    ...
    async initPoolIdx (poolIdx: BigNumberish, feeRate: number, protoTake: number, tickSize: number,
        ...
        await this.setupLpTokenBeaconAddress(await this.dex)
    ...
    async encodeInitPool (poolIdx: BigNumberish, price:number | BigNumber): Promise<BytesLike> {
        ...
        return abiCoder.encode(["uint8", "address", "address", "uint256", "uint128"],
                [71, (await this.base).address, (await this.quote).address, poolIdx, this.toSdexPrice(price)])
    }

    toSdexPrice (price: number | BigNumber): BigNumber {
