...
import { MockSdexNonceOracle } from '../typechain/MockSdexNonceOracle';
...
describe('Pool Relayer Agent', () => {
    let test: TestPool
    let dex: string
    let oracle: MockSdexNonceOracle
    let baseToken: Token
    let quoteToken: Token
    let sender: string
    let other: string
    ...
    beforeEach("deploy",  async () => {
       ...
       let factory = await ethers.getContractFactory("MockSdexNonceOracle")
       oracle = await (factory.deploy() as Promise<MockSdexNonceOracle>)
    })
