...
describe('SdexPolicy', () => {
   ...
   beforeEach("deploy", async () => {
      ...
      policy = (await factory.deploy(minion.address)) as SdexPolicy;
      ...
      expect(await policy.ownedByGovernance()).to.be.eq(false);
      policy.transferGovernance(ops.address, treasury.address, emergency.address);
      expect(await policy.ownedByGovernance()).to.be.eq(true);
    })
    ...
    it("transferOwnership should failed if ownership has been transferred to governance already", async() => {
        const opsAddress = accts[1].address;
        const treasuryAddress = accts[1].address;
        const emergencyAddress = accts[1].address;
        await expect(policy.transferOwnership(opsAddress, treasuryAddress, emergencyAddress)).to.be.revertedWith("Owned by governance already");
    })
    ...
    it("transfer not accepted", async() => {
        ...
        // Will reject because the MockTimelock is not set to accept SdexPolicy address
...
describe('SdexPolicy transferOwnership', () => {
    let policy: SdexPolicy
    let minion: MockMinion
    let accts: Wallet[]
    let ops: MockTimelock
   ...
   beforeEach("deploy", async () => {
    ...
    it("transferOwnership should failed if called by non-treasury address", async() => {
        const opsAddress = accts[1].address;
        const treasuryAddress = accts[1].address;
        const emergencyAddress = accts[1].address;
        await expect(policy.connect(accts[1]).transferOwnership(opsAddress, treasuryAddress, emergencyAddress)).to.be.revertedWith("Treasury Authority");
    })

    it("transferOwnership should failed if try to set to 0 address", async() => {
        const opsAddress = accts[1].address;
        const treasuryAddress = accts[1].address;
        const emergencyAddress = accts[1].address;
        await expect(policy.connect(accts[0]).transferOwnership(ethers.constants.AddressZero, treasuryAddress, emergencyAddress)).to.be.revertedWith("non-zero address");
        await expect(policy.connect(accts[0]).transferOwnership(opsAddress, ethers.constants.AddressZero, emergencyAddress)).to.be.revertedWith("non-zero address");
        await expect(policy.connect(accts[0]).transferOwnership(opsAddress, treasuryAddress, ethers.constants.AddressZero)).to.be.revertedWith("non-zero address");
    })
    ...
    it("should transferOwnership successfully", async() => {
        ...
        expect(await policy.dex_()).to.be.eq(minion.address)
        expect(await policy.opsAuthority_()).to.be.eq(opsAddress)
        expect(await policy.treasuryAuthority_()).to.be.eq(treasuryAddress)
        expect(await policy.emergencyAuthority_()).to.be.eq(emergencyAddress)
